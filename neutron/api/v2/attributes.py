...
def _validate_portforwardings(data, valid_values=None):
    # TODO(liuzhikun): add function to filter the duplicate range
    if not isinstance(data, list):
    ...
    for portfwd in data:
        ...
        # validate outside port value 'xxx-xxx' or 'xxx'
        o_list = portfwd['outside_port'].split('-')
        for p in o_list:
            msg = _validate_range(p, (0, 65535))
            if msg:
                LOG.debug(msg)
                return msg
        ...
        # validate inside port value 'xxx-xxx' or 'xxx'
        i_list = portfwd['inside_port'].split('-')
        for p in i_list:
            msg = _validate_range(p, (0, 65535))
            if msg:
                LOG.debug(msg)
        ...
        if len(o_list) > 1 and o_list != i_list:
            msg = _("Invalid port range for portforwarding: '%s'") % data
            LOG.debug(msg)
            return msg
        ...
        msg = _validate_values(portfwd['status'].upper(), ('ENABLE', 'DISABLE'))
        if msg:
            LOG.debug(msg)
            return msg
