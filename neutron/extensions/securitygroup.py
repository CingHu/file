...
class SecurityGroupInvalidPriorityValue(qexception.InvalidInput):
    message = _("Invalid value for priority %(priority)s")
...
def convert_validate_priority_value(priority):
    if priority is None:
        return priority
    try:
        val = int(priority)
    except (ValueError, TypeError):
        raise SecurityGroupInvalidPriorityValue(priority=priority)
...
RESOURCE_ATTRIBUTE_MAP = {
    ...
    'security_group_rules': {
        ...
        'priority': {'allow_post': True, 'allow_put': True,
                           'convert_to': convert_validate_priority_value,
                           'default': None, 'is_visible': True},
