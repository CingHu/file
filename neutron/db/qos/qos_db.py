...
class NetworkQoSMapping(model_base.BASEV2):
    ...
    # to eagerly load qos mapping
    networks = orm.relationship(
        models_v2.Network,
        backref=orm.backref("qos_mapping",
...
class QoSDbMixin(ext_qos.QoSPluginBase):
    ...
    def _extend_port_dict_qos(self, port_res, port_db):
        # QoS will be retrieved from the sqlalchemy
        # model. As they're loaded eagerly with ports because of the
        # joined load they will not cause an extra query.
        qos_id = [qos_mapping['qos_id'] for
                        qos_mapping in port_db.qos_mapping]
        port_res[ext_qos.QOS] = qos_id
        return port_res

    # Register dict extend functions for ports
    db_base_plugin_v2.NeutronDbPluginV2.register_dict_extend_funcs(
        attr.PORTS, ['_extend_port_dict_qos'])

    def _extend_network_dict_qos(self, network_res, network_db):
        # QoS will be retrieved from the sqlalchemy
        # model. As they're loaded eagerly with networks because of
        # the joined load they will not cause an extra query.
        qos_id = [qos_mapping['qos_id'] for
                        qos_mapping in network_db.qos_mapping]
        network_res[ext_qos.QOS] = qos_id
        return network_res

    # Register dict extend functions for networks
    db_base_plugin_v2.NeutronDbPluginV2.register_dict_extend_funcs(
        attr.NETWORKS, ['_extend_network_dict_qos'])
    ...
    def create_qos_for_port(self, context, qos_id, port_id, ip_address):
        with context.session.begin(subtransactions=True):
            db = PortQoSMapping(qos_id=qos_id, port_id=port_id, ip_address=ip_address)
            context.session.add(db)
        return db.qos_id
    ...
    def delete_qos_for_port(self, context, port_id, ip_address):
        try:
            self._db_delete(context,
    ...
    def get_mapping_for_port(self, context, port_id, ip_address):
        try:
            with context.session.begin(subtransactions=True):
    ...
    def update_mapping_for_port(self, context, mapping):
        db = self.get_mapping_for_port(context,
                                       mapping.port_id,
                                       mapping.ip_address)[0]
        with context.session.begin(subtransactions=True):
            db.update(mapping)
    ...
    def validate_qos(self, qos):
        # TODO(liuzhikun) fix validate_qos
        return
