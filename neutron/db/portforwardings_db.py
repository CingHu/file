...
class PortForwardingRule(model_base.BASEV2, models_v2.HasId):
    ...
    router = orm.relationship(l3_db.Router,
                              backref=orm.backref("portforwarding_list",
                                                  lazy='joined',
                                                  cascade='delete'))
    outside_port = sa.Column(sa.String(11))
    inside_addr = sa.Column(sa.String(15))
    inside_port = sa.Column(sa.String(11))
    # NOTE: status is "enable" or "disable"
    status = sa.Column(sa.String(8))
    # NOTE: protocol can be either TCP or UDP
...
class Port_Forwarding_Db_Mixin(l3_db.L3_NAT_db_mixin):
    ...
    def _extend_router_dict_portforwarding(self, router_res, router_db):
        router_res['portforwardings'] = (
            Port_Forwarding_Db_Mixin._make_extra_portfwd_list(
    ...
    def update_router(self, context, id, router):
        ...
        router_updated = super(Port_Forwarding_Db_Mixin, self).update_router(
            context, id, router)
    ...
    def _update_extra_portfwds(self, context, router, portfwds):
        ...
        # NOTE: remove first so that we won't encounter duplicated
        #       entry.
