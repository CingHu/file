...
class SecurityGroupDbMixin(ext_sg.SecurityGroupPluginBase):
    ...
    def _make_security_group_dict(self, security_group, fields=None):
        ...
        res['security_group_rules'] = sorted(security_group_rules, key=lambda x:x['priority'],reverse=True)
    ...
    def _validate_security_group_rules_update(self, context, rule):
        """Check that rules being installed.
    ...
    def _make_security_group_rule_dict(self, security_group_rule, fields=None):
        res = {'id': security_group_rule['id'],
               ...
               'enabled': security_group_rule['enabled'],
               'remote_ip_prefix': security_group_rule['remote_ip_prefix'],
               'remote_group_id': security_group_rule['remote_group_id']}

        return self._fields(res, fields)

    def _make_security_group_rule_dict_update(self, db_sg_rule, security_group_rule, fields=None):
        res = {'id': security_group_rule.get('id',None) or db_sg_rule['id'],
               'tenant_id': security_group_rule.get('tenant_id',None) or db_sg_rule['tenant_id'],
               'security_group_id': security_group_rule.get('security_group_id',None) or  db_sg_rule['security_group_id'],
               'ethertype': security_group_rule.get('ethertype',None) or  db_sg_rule.get('ethertype',None),
               'direction': security_group_rule.get('direction',None) or  db_sg_rule.get('direction',None),
               'protocol': security_group_rule.get('protocol',None) or   db_sg_rule.get('protocol',None),
               'port_range_min': security_group_rule.get('port_range_min',None) or  db_sg_rule.get('port_range_min',None),
               'port_range_max': security_group_rule.get('port_range_max',None) or  db_sg_rule.get('port_range_max', None),
               'priority':security_group_rule.get('priority',None) or  db_sg_rule.get('priority', 0),
               'enabled': security_group_rule.get('enabled'),
               'remote_ip_prefix': security_group_rule.get('remote_ip_prefix',None) or  db_sg_rule.get('remote_ip_prefix', None),
               'remote_group_id': security_group_rule.get('remote_group_id',None) or  db_sg_rule.get('remote_group_id',None)}
    ...
    def update_security_group_rule(self, context, id, security_group_rule):
        ...
        db_sg_rule = self.get_security_group_rule(context, id)
        rule = self._make_security_group_rule_dict_update(db_sg_rule, r)
        self._validate_security_group_rules_update(context, rule)
        scoped_session(context.session)
        with context.session.begin(subtransactions=True):
            self._check_for_duplicate_rules(context, [{'security_group_rule':rule}])
    ...
    def _is_sg_port(self, context, port):

        if port.get('device_owner') is None:
            return False

        elif port.get('device_owner').startswith('network:') and \
            port['device_owner'] != constants.DEVICE_OWNER_ROUTER_GW and \
                 port['device_owner'] != constants.DEVICE_OWNER_LOADBALANCER:
            return True
    ...
    def _ensure_default_security_group_on_port(self, context, port):
        ...
        tenant_id = self._get_tenant_id_for_create(context,
                                                   port['port'])
        if self._is_sg_port(context, port['port']):
             return
