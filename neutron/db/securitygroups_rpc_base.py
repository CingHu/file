...
class SecurityGroupServerRpcMixin(sg_db.SecurityGroupDbMixin):
    ...
    def update_security_group_rule(self, context,sgrid, security_group_rule):
        rule = self.get_security_group_rule(context, sgrid)
        super(SecurityGroupServerRpcMixin,
              self).update_security_group_rule(context, sgrid, security_group_rule)
        self.notifier.security_groups_rule_updated(context,
                                                   [rule['security_group_id']])
    ...
    def security_group_info_for_ports(self, context, ports):
        sg_info = {'devices': ports,
                   'security_groups': {},
                   'sg_member_ips': {}}
        rules_in_db = self._select_sorted_rules_for_ports(context, ports)
        ...
        for (binding, rule_in_db) in rules_in_db:
            ...
            for key in ('protocol', 'port_range_min', 'port_range_max',
                        'remote_ip_prefix', 'remote_group_id','priority','enabled'):
    ...
    def _select_sorted_rules_for_ports(self, context, ports):
        rules = self._select_rules_for_ports(context, ports)
        return sorted(rules, key=lambda x:x[1]['priority'],reverse=True)
    ...
    def _select_rules_for_ports(self, context, ports):
        ...
        query = query.filter(sg_db.SecurityGroupRule.enabled==True)
