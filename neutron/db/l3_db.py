...
class L3_NAT_dbonly_mixin(l3.RouterPluginBase):
    ...
    def _create_router_gw_port(self, context, router, network_id, gwinfo=None):
        gw_fixed_ips = attributes.ATTR_NOT_SPECIFIED
        if(gwinfo and gwinfo.has_key('external_fixed_ips')):
            gw_fixed_ips = gwinfo['external_fixed_ips']
        # Port has no 'tenant-id', as it is hidden from user
        gw_port = self._core_plugin.create_port(context.elevated(), {
            'port': {'tenant_id': '',  # intentionally not set
                     'network_id': network_id,
                     'mac_address': attributes.ATTR_NOT_SPECIFIED,
                     'fixed_ips': gw_fixed_ips,
                     'device_id': router['id'],
    ...
    def _delete_current_gw_port(self, context, router_id, router, new_network,
        ...
        is_new_ips = False
        if(gwinfo and gwinfo.has_key('external_fixed_ips')):
            ext_fixed_ips = gwinfo['external_fixed_ips']
            if (ext_fixed_ips and ext_fixed_ips[0].has_key('subnet_id') and
                   ext_fixed_ips[0].has_key('ip_address')):
                is_new_ips = True
        is_gw_port_attached_to_existing_network = (
            router.gw_port and router.gw_port['network_id'] != new_network)
        admin_ctx = context.elevated()
        if is_gw_port_attached_to_existing_network or (is_new_ips and router.gw_port):
    ...
    def _create_gw_port(self, context, router_id, router, new_network,
        ...
        gw_info = gwinfo
        is_new_ips = False
        if(gwinfo != None and gwinfo.has_key('external_fixed_ips')):
            ext_fixed_ips = gwinfo['external_fixed_ips']
            if (ext_fixed_ips != None and ext_fixed_ips[0].has_key('subnet_id')
                and ext_fixed_ips[0].has_key('ip_address')):
                is_new_ips = True
        new_valid_gw_port_attachment = (
            new_network and (not router.gw_port or
                             router.gw_port['network_id'] != new_network))
        if new_valid_gw_port_attachment or is_new_ips:
            ...
            for subnet in subnets:
                self._check_for_dup_router_subnet(context, router,
                                                  ...
                                                  subnet['cidr'])
            LOG.debug("CHINAC: create gw port called")
            self._create_router_gw_port(context, router, new_network, gwinfo=gw_info)
    ...
    def _update_router_gw_info(self, context, router_id, info, router=None):
        ...
        self._delete_current_gw_port(context, router_id, router, network_id, gwinfo=info)
        self._create_gw_port(context, router_id, router, network_id, gwinfo=info)
